...
// 2024 PCCP 기출문제 2번 석유시추(Lv.2) 문제이며 1번부터 m번까지의 땅에서 단 하나의 시추관으로 석유를 시추할 때 최대량을 출력하는 문제이다.
// 매개변수로 석유가 매장되어있는 땅을 2차원배열(이하 land)이며 연결되어있는 석유를 찾은 후 해당 번호의 땅에 합산 후 최대량을 출력해야하므로
// 연결되어있는 석유를 찾기위한 탐색으로 DFS 또는 BFS을 사용해야하고 필자는 BFS를 사용하여 풀이하였다.
// 하지만 문제 효율성조건으로 땅의 넓이의 최대치는 500*500 이므로 중복탐색을 지양해야함. set과 visit을 이용하여 m번까지의 중복탐색을 지양하였다.
